...
class PersonTracker:
    def __init__(self):
        ...
        self.missing_threshold = 10  # 사람이 사라졌다고 판단할 프레임 수
...
def initialize_map(width, height):
    ...
    # 사각형 맵의 크기 (250cm x 250cm)
    map_size = min(width, height) - 40  # 여백 40px
    
    # 사각형 맵의 코너 좌표 계산
    top_left = (center[0] - map_size // 2, center[1] - map_size // 2)
    bottom_right = (center[0] + map_size // 2, center[1] + map_size // 2)
    
    # 사각형 경계 그리기
    cv2.rectangle(map_frame, top_left, bottom_right, (0, 0, 0), 2)
    
    # 그리드 라인 그리기 (45cm 간격)
    # 250cm를 45cm로 나누면 약 5.55개의 그리드가 됨, 균등하게 5개로 조정
    grid_count = 5
    grid_spacing = map_size // grid_count
    
    # 수직선
    for i in range(1, grid_count):
        x = top_left[0] + i * grid_spacing
        cv2.line(map_frame, (x, top_left[1]), (x, bottom_right[1]), (200, 200, 200), 1)
    
    # 수평선
    for i in range(1, grid_count):
        y = top_left[1] + i * grid_spacing
        cv2.line(map_frame, (top_left[0], y), (bottom_right[0], y), (200, 200, 200), 1)
    
    # 맵 축척 표시
    cv2.putText(map_frame, "250cm x 250cm", (top_left[0], top_left[1] - 10),
               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
    cv2.putText(map_frame, "Grid: 45cm", (top_left[0], top_left[1] - 30),
               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
    
    return map_frame, center, map_size
...
def main():
    ...
    # 맵 초기화
    map_frame, map_center, map_size = initialize_map(viz_width, viz_height)
    ...
    try:
        while True:
            ...
            for person_id, pos in tracked_persons.items():
                ...
                # 유효한 깊이 값이 있는 경우만 처리
                if depth_value > 0 and depth_value < 2.5:  # 2.5m(250cm) 환경에 맞게 조정
                    # 3D 공간상의 위치 계산
                    ...
                    x_pos = depth_point[0]  # 좌우 위치 (미터)
                    z_pos = depth_point[2]  # 깊이 (미터)
                    
                    # 맵의 좌표로 변환 (250cm 맵에 맞게 조정)
                    # 맵의 크기는 2.5m(250cm)
                    if abs(x_pos) < 1.25 and z_pos < 2.5 and z_pos > 0:  # 맵 범위 내에 있는 경우만 표시
                        # x, z 좌표를 맵의 좌표로 변환
                        map_x = int(map_center[0] + (x_pos / 1.25) * (map_size / 2))
                        map_y = int(map_center[1] - (z_pos / 2.5) * map_size)
