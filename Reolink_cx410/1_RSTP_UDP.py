...
def main():
    ...
    unity_address = ('127.0.0.1', 5065)  # Unity 앱이 실행되는 IP와 포트
    print(f"UDP 소켓 생성: {unity_address}")
    
    # RTSP URL 설정
    #rtsp_url = "rtsp://admin:RELIQUUM0925@192.168.0.3:554/Preview_01_sub"  # 실제 RTSP URL로 변경 필요

    # Logitech C270 웹캠 사용
    camera_index = 0  # 일반적으로 첫 번째 웹캠은 0, 필요에 따라 변경
    print(f"웹캠 연결 중: 카메라 인덱스 {camera_index}")
    ...
    while True:
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                try:
                    ...
                    # 위치 정보를 UDP로 Unity에 전송
                    position_data = f"{real_x:.1f},{real_y:.1f}"
                    udp_socket.sendto(position_data.encode(), unity_address)
                    
                    # UDP 패킷 전송 통계 업데이트
                    packets_sent += 1
                    last_sent_time = cv2.getTickCount() / cv2.getTickFrequency()
        ...
        # UDP 상태 표시
        udp_status = f"UDP: {packets_sent} packets sent"
        cv2.putText(display_image, udp_status, (10, 60), 
